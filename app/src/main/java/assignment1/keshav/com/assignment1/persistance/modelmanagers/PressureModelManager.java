 ...
 */
public class PressureModelManager extends SensorModelManager
{
    ...
    public static final String DELETE_TABLE = "DROP TABLE IF EXISTS " + PressureEntry.TABLE_NAME;

    public static final String CREATE_TABLE =
            "CREATE TABLE IF NOT EXISTS " + PressureEntry.TABLE_NAME + " (" +
                    PressureEntry._ID + INT_TYPE + " PRIMARY KEY, " +
                    PressureEntry.AXIS_X + REAL_TYPE + COMMA_SEP +
                    PressureEntry.TIME_TAKEN + TIMESTAMP + ") ;";

    public PressureModelManager(SQLiteDatabase db) {
        super(db);
    ...
    public boolean saveEntry(float[] values) {
        ...
        cv.put(PressureEntry.AXIS_X, values[0]);
        cv.put(PressureEntry.TIME_TAKEN, dFormat.format(new Date()));

        long res = this.db.insert(PressureEntry.TABLE_NAME, null, cv);
        return (res != -1);
    }
    ...
    public LineData retrieveChartData() {
        ...
        stb.append("SELECT * FROM ")
                .append(PressureEntry.TABLE_NAME)
                .append(" ORDER BY " + PressureEntry.TIME_TAKEN + " DESC ")
                .append(LIMIT);
        ...
        while(res.moveToNext())
        {
            ...
            // Add entries
            xEntries.add(new Entry(res.getFloat(res.getColumnIndex(PressureEntry.AXIS_X)), count));
