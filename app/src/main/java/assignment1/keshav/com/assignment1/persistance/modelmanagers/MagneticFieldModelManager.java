...
public class MagneticFieldModelManager extends SensorModelManager
{
    ...
    public MagneticFieldModelManager(SQLiteDatabase db) {
        super(db);
    ...
    public boolean saveEntry(float[] values) {
        ...
        cv.put(MagneticFieldrEntry.AXIS_X, values[0]);
        cv.put(MagneticFieldrEntry.AXIS_Y, values[1]);
        cv.put(MagneticFieldrEntry.AXIS_Z, values[2]);
        cv.put(MagneticFieldrEntry.TIME_TAKEN, dFormat.format(new Date()));

        long res = this.db.insert(MagneticFieldrEntry.TABLE_NAME, null, cv);
        return (res != -1);
    }
    ...
    public LineData retrieveChartData() {
        ...
        stb.append("SELECT * FROM ")
                .append(MagneticFieldrEntry.TABLE_NAME)
                .append(" ORDER BY " + MagneticFieldrEntry.TIME_TAKEN + " DESC ")
                .append(LIMIT);
        ...
        while(res.moveToNext())
        {
            ...
            // Add entries
            xEntries.add(new Entry(res.getFloat(res.getColumnIndex(MagneticFieldrEntry.AXIS_X)), count));
            yEntries.add(new Entry(res.getFloat(res.getColumnIndex(MagneticFieldrEntry.AXIS_Y)), count));
            zEntries.add(new Entry(res.getFloat(res.getColumnIndex(MagneticFieldrEntry.AXIS_Z)), count));
